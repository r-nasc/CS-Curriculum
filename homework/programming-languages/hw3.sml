...
fun longest_string_helper dlg lst = foldl (fn(x,y) => if dlg(String.size x, String.size y) then x else y) "" lst
...
fun count_some_var(str, ptrn) = g (fn x => 0) (fn x => if x = str then 1 else 0) ptrn
...
fun first_match v ptrnlist = SOME(first_answer (fn p => match(v, p)) ptrnlist) handle NoAnswer => NONE
